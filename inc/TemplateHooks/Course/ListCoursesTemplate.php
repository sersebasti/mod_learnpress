...
class ListCoursesTemplate {
	...
	public static function render_course( $course, array $settings = [] ): string {
		...
		try {
            ...
            // GEt custom field start date and end date
			$start_date_raw = get_field('lp_start_date', $course->get_id());
			$end_date_raw   = get_field('lp_end_date', $course->get_id());

			$start_date = $start_date_raw ? date('d/m/Y', strtotime($start_date_raw)) : '';
			$end_date   = $end_date_raw ? date('d/m/Y', strtotime($end_date_raw)) : '';
			...
			$section_bottom_end = apply_filters(
				...
				[
					...
					//'price' => '<span class="course-price">Prezzo: 1 €</span>',
